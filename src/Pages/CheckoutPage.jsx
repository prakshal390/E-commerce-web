...
const CheckoutPage = () => {
  ...
  // ================= FORM STATE =================
  const [formData, setFormData] = useState(() => {
    const savedForm = localStorage.getItem("checkout_formData");
    return savedForm
  ...
  const [paymentMethod, setPaymentMethod] = useState(() => {
    return localStorage.getItem("checkout_paymentMethod") || "upi";
  });

  const [upiId, setUpiId] = useState(() => {
    return localStorage.getItem("checkout_upiId") || "";
  });

  const [cardData, setCardData] = useState(() => {
    const savedCard = localStorage.getItem("checkout_cardData");
    return savedCard
  ...
  const [bankName, setBankName] = useState(() => {
    return localStorage.getItem("checkout_bankName") || "";
  });
  ...
  useEffect(() => {
    localStorage.setItem("checkout_upiId", upiId);
  }, [upiId]);
  ...
  useEffect(() => {
    localStorage.setItem("checkout_bankName", bankName);
  }, [bankName]);
  ...
  const clearCheckoutStorage = () => {
    ...
    localStorage.removeItem("checkout_upiId");
    localStorage.removeItem("checkout_cardData");
    localStorage.removeItem("checkout_bankName");
  };
  ...
  const handlePlaceOrder = (e) => {
    ...
    // order place hone ke baad fields reset + localStorage clear
    setFormData({
      fullName: "",
      email: "",
      phone: "",
